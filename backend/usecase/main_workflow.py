...
class MainWorkflow:
    ...
    async def process_user_input(self, user_message: str, session_id: str) -> Dict[str, Any]:
        ...
        if current_state == SessionState.INITIAL.value:
            return await self._route_to_extractor(session_id, session_state, user_message)
        elif current_state == SessionState.CLARIFYING.value:
            return await self._handle_clarification(session_id, session_state, user_message)
            ...
            return await self._handle_post_advice(session_id, session_state, user_message)
        else:
            return {"error": f"Unknown session state: {current_state}"}
    
    async def _route_to_extractor(self, session_id: str, session_state: Dict, user_message: str) -> Dict[str, Any]:
    ...
    async def _route_to_search_agent(self, session_id: str, session_state: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # Call search agent
            search_result = await self.food_search_agent.arun(search_payload, input_schema=FoodSearchPayload)
            ...
            if self._is_search_complete(search_result):
                ...
                return await self._route_to_advisor(session_id, session_state, search_result)
            else:
    ...
    async def _handle_post_advice(self, session_id: str, session_state: Dict, user_message: str) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    async def test_workflow():
        ...
        result1 = await workflow.process_user_input("Sarapan: nasi goreng, Lunch: soto ayam", session_id)
        print(result1)
