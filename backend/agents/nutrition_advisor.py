...
from models.models import DailyMealData, FoodItem
...
class NutritionAdvice(BaseModel):
    ...
    strengths: List[str] = Field(
        description="Positive aspects of the diet"
    )
    ...
    macro_balance_score: int = Field(
        description="Score from 1-10 rating the macronutrient balance",
        ge=1,
        le=10
    )
    meal_distribution_score: int = Field(
        description="Score from 1-10 rating how well meals are distributed",
        ge=1,
        le=10
    )
...
if __name__ == "__main__":
    ...
    for meal_type, nutrients in analysis.summary.meals_breakdown.items():
        ...
        print(f"  P: {nutrients['protein']:.1f}g | C: {nutrients['carbohydrates']:.1f}g | F: {nutrients['fat']:.1f}g")
